...
const ALGORITHM_ID = 'value-iteration';
...
function ValueIteration() {
    ...
    const handleStart = useCallback(async () => {
        ...
        try {
            ...
            const session = await api.createSession(
                { type: envType, rows, cols, maxSteps: 100, mapName: envConfig.mapName || '4x4', isSlippery: envConfig.isSlippery !== false },
    ...
    }, [rows, cols, params, animationSpeed, doStep]);
    ...
    return (
        <div className="algorithm-page">
            <header className="page-header">
                ...
                <p>Compute optimal values using Bellman optimality updates</p>
            </header>
            <div className="algorithm-description">
                <h2>Algorithm Description</h2>
                <p>Directly computes optimal value function V*:</p>
                <div className="formula">V(s) = max<sub>a</sub> [R(s,a) + γ × V(s')]</div>
            </div>
            <div className="page-content">
                    ...
                    <div className="control-buttons">
                        {!isRunning ? (
                            <button className="run-button" onClick={handleStart} disabled={converged}>
                                {converged ? '✓ Converged' : '▶ Run Value Iteration'}
                            </button>
                        ) : (
                ...
                <section className="diagnostics-section">
                    ...
                    <div className="diagnostics-placeholder">
                        <div className="diagnostic-item"><span className="label">Iterations</span><span className="value">{iteration}</span></div>
                        <div className="diagnostic-item"><span className="label">Final ΔV</span><span className="value">{diagnostics?.max_delta?.toFixed(6) || '-'}</span></div>
                        <div className="diagnostic-item"><span className="label">Converged</span><span className="value">{converged ? '✓ Yes' : 'No'}</span></div>
                    </div>
                </section>
            </div>
        </div>
